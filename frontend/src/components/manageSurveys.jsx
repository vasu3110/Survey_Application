...
const ManageSurveys = () => {
    const { formTypes, fetchSurveys, createSurvey, updateSurvey, deleteSurvey } = useContext(FormContext);
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {formTypes.map((survey) => (
                            <tr key={survey.id}>
                                <td className="px-6 py-4 whitespace-nowrap font-mono text-sm">{survey.id}</td>
                                <td className="px-6 py-4 whitespace-nowrap">{survey.name}</td>
                                <td className="px-6 py-4 whitespace-nowrap">{new Date(survey.createdAt).toLocaleString()}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <div className="flex justify-center space-x-2">
                                        <button onClick={() => handleOpenViewModal(survey)} className="text-gray-600 hover:text-gray-900" title="View"><Eye size={18} /></button>
                                        <button onClick={() => handleOpenUpdateModal(survey)} className="text-blue-600 hover:text-blue-900" title="Update"><Edit size={18} /></button>
                                        <button onClick={() => handleDelete(survey.id)} className="text-red-600 hover:text-red-900" title="Delete"><Trash2 size={18} /></button>
                                    </div>
                                </td>
                            </tr>
                        ))}
